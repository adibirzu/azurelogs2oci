...
resource "oci_streaming_stream" "azure_stream" {
  ...
  stream_pool_id     = oci_streaming_stream_pool.azure_pool.id
...
resource "oci_sch_service_connector" "azure_bridge" {
  ...
  source {
    ...
    stream_id = oci_streaming_stream.azure_stream.id
  }

  target {
    kind                  = "loggingAnalytics"
    log_group_id          = oci_log_analytics_log_analytics_log_group.azure_logs.id
    log_source_identifier = "Azure EntraID Audit Logs"
  }
}
